...
Defines the `defineBuildConfig` used by the `build.config.mjs` configuration file.
...
-   Service required by the package
...
The framework's job is thus, starting from an application, to detect the application's dependencies, gather their metadata and prepare all information
...
The build plugin implements the `resolveId` and `load` vite/rollup-plugin hooks (see [Documentation](https://rollupjs.org/plugin-development/#load))
in order to provide a number of virtual modules (the most important user facing one is `"open-pioneer:app"`).
When it is triggered by an import such as the one in the example above, it detects the _location_ of the input (the package that app.js belongs to).

From there on, it can trigger the analysis of the package itself (read package.json, build.config.js) and its entire dependency tree
...
It will return a file such as the (simplified) one below, which delegates the actual code generation to other, also virtual modules.
...
```js
// content of "open-pioneer:app" for a specific application
...
The build plugin determines the set of supported languages for an app by inspecting the apps's `build.config.mjs`.
